                    ...
                    <div class="card">
                        ...
                        <div class="card-header d-flex justify-content-between">
                            <div class="iq-header-title">
                                <h4 class="card-title mb-0">Counselor List</h4>
                            </div>
                            <a href="{{route('counselor.create')}}" class="btn btn-primary">Add New</a>
                            ...
                            @if (count($data) > 0)
                                <div class="table-responsive">
                                    ...
                                    <table class="table data-table" style="width:100%">
                                        ...
                                        <tbody>
                                            @foreach ($data as $key => $counselor)
                                                <tr>
                                                    <td>{{++$key}}</td>
                                                    <td>
                                                        <img src="{{ asset('images/counselor/'.$counselor->picture) }}" onerror="this.onerror=null;this.src='{{ asset('admin/images/user/user-dummy-img.png') }}';" alt="{{ $counselor->first_name }}"  class="rounded avatar-40 img-fluid" >
                                                    </td>
                                                    <td>{{$counselor->first_name}} {{$counselor->last_name}}</td>
                                                    <td>{{$counselor->email}}</td>
                                                    <td>{{$counselor->mobile_number}}</td>
                                                    <td>{!! getGeneralStatus($counselor->status, 'badge') !!}</td>
                                                    <td>
                                                        <div class="d-flex align-items-center list-action">
                                                            <a class="badge bg-warning-light mr-2" data-toggle="tooltip"
                                                                data-placement="top" title="" data-original-title="Edit"
                                                                href="{{ route('counselor.edit', $counselor->id) }}">
                                                                <i class="fa fa-pen"></i>
                                                            </a>
                                                            {{-- <a class="badge bg-danger-light mr-2 rtl-ml-2" data-toggle="modal tooltip"
                                                                ...
                                                                href="#deleteRecord{{$counselor->id}}"
                                                                href="#" id="deleteOrder" delete-id="{{$counselor->id}}">
                                                                ...
                                                                <div class="modal-dialog modal-sm" bis_skin_checked="1">
                                                                    <div class="modal-content" bis_skin_checked="1">
                                                                        ...
                                                                        <div class="modal-footer" bis_skin_checked="1">
                                                                            <form method="POST" action="{{ route('counselor.destroy', ['consular' => $counselor->id]) }}">
                                                                                @method('DELETE')
                                                                                @csrf
